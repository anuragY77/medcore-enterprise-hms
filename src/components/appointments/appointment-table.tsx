...
type SortField = "appointmentId" | "doctorName" | "department" | "date" | "type" | "status";
...
export function AppointmentTable({ appointments, pageSize = 10 }: AppointmentTableProps) {
            ...
            <tr className="border-b border-border/50 bg-muted/30">
              ...
              >
                <div className="flex items-center gap-1.5">
                  ID <SortIcon field="appointmentId" currentSort={sortField} direction={sortDirection} />
                </div>
              </th>
              <th
                className="text-left px-4 py-3 font-medium text-muted-foreground cursor-pointer hover:text-foreground transition-colors"
              ...
              >
                <div className="flex items-center gap-1.5">
                  Doctor <SortIcon field="doctorName" currentSort={sortField} direction={sortDirection} />
                </div>
              </th>
              <th
                className="text-left px-4 py-3 font-medium text-muted-foreground cursor-pointer hover:text-foreground transition-colors"
              ...
              >
                <div className="flex items-center gap-1.5">
                  Date <SortIcon field="date" currentSort={sortField} direction={sortDirection} />
                </div>
              </th>
              ...
              >
                <div className="flex items-center gap-1.5">
                  Type <SortIcon field="type" currentSort={sortField} direction={sortDirection} />
                </div>
              </th>
              <th
                className="text-left px-4 py-3 font-medium text-muted-foreground cursor-pointer hover:text-foreground transition-colors"
            ...
            {paginatedAppointments.length === 0 ? (
              ...
              paginatedAppointments.map((apt) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-medium text-foreground">{apt.doctorName}</td>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={cn(
                        ...
                        STATUS_COLORS[apt.status] ?? "bg-slate-100 text-slate-800"
                      )}
                    >
                      {apt.status}
                    </span>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-4 py-3 border-t border-border/50">
          <p className="text-xs text-muted-foreground">
            Showing {startIndex + 1}–{Math.min(startIndex + pageSize, sortedAppointments.length)} of{" "}
